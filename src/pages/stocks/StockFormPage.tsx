...
export function StockFormPage() {
  ...
  const submit = (values: StockFormValues) => {
    ...
    void navigate('/warehouse/stocks');
  };
  ...
  if (isEditing && !existingStock) {
    return (
      <div className="page-stack">
        ...
        <Button variant="secondary" onClick={() => void navigate('/warehouse/stocks')}>Back to stock list</Button>
      </div>
    );
  }
  ...
  return (
    <div className="page-stack">
      <PageHeader
        ...
        description="Create a reusable stock master record. Quantity is updated separately when goods are received."
      />
      ...
      <form onSubmit={handleSubmit(submit)} noValidate>
        ...
        >
          ...
          <div className="form-actions">
            ...
            >
              {isEditing ? 'Save changes' : 'Save stock entry'}
            </Button>
            <Button
              variant="secondary"
              icon={<XCircle size={17} />}
              onClick={() => void navigate('/warehouse/stocks')}
            >
              Cancel
            </Button>
            {!isEditing ? (
              <Button variant="ghost" onClick={() => reset(defaultValues)}>Clear</Button>
            ) : null}
          </div>
        </Card>
      </form>
