...
export function PurchasingSupplierFormPage() {
  const { supplierCode } = useParams();
  const isEditing = Boolean(supplierCode);
  const navigate = useNavigate();
  const { state, dispatch } = useAppData();
  const { notify } = useNotification();
  const existingSupplier = state.suppliers.find((supplier) => supplier.supplierCode === supplierCode);
  ...
  const submit = (values: SupplierFormValues) => {
    ...
    void navigate('/purchasing/suppliers');
  };
  ...
  if (isEditing && !existingSupplier) {
    return (
      <div className="page-stack">
        ...
        <Button variant="secondary" onClick={() => void navigate('/purchasing/suppliers')}>Back to suppliers</Button>
      </div>
    );
  }
          ...
          <Card title="Bank details" description="Store payment details as reference data only.">
            ...
            <div className="form-actions">
              ...
              <Button
                ...
                onClick={() => void navigate('/purchasing/suppliers')}
              >
                Cancel
              </Button>
